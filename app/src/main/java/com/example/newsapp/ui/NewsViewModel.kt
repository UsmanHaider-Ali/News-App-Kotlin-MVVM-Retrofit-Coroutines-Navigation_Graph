...
class NewsViewModel(app: Application, val newsRepository: NewsRepository) : AndroidViewModel(app) {
    ...
    val newsSearch: MutableLiveData<Resource<NewsResponse>> = MutableLiveData()
    ...
    private suspend fun searchNewsInternet(searchQuery: String) {
        ...
        try {
            if (internetConnection(this.getApplication())) {
                ...
                newsSearch.postValue(handleSearchNewsResponse(response))
            } else {
                newsSearch.postValue(Resource.Error("No Internet Connection"))
            }
        } catch (t: Throwable) {
            when (t) {
                is IOException -> newsSearch.postValue(Resource.Error("Unable to Connect"))
                else -> newsSearch.postValue(Resource.Error("No Signal"))
            }
        }
    }
